...
<a href="admin.php?page=<?php echo XY_ALBUM_DIR?>/xy_album_admin.php&amp;allow_create_album=true">创建相册</a>	
<?php	
}
?>
<br />
	...
	<!--上载新照片-->
   	<form action= "admin.php?page=<?php echo XY_ALBUM_DIR?>/xy_album_admin.php&amp;action_file_upload=true,&amp;allow_file_loading=true" id="form_upload" method="post" enctype="multipart/form-data"/>	
   		<table border="1">
    	<div>
     	<input type="file" name="file" id="form_upload_name" readonly="" />
     	<br/>
     	<!--下面是显示描述文本框，有灰色提示语言功能-->
     	<textarea name="description" id="form_file_desc" rows=2 cols=60   onFocus="if(value==defaultValue){value='';this.style.color='#000'}" onBlur="if(!value){value=defaultValue;this.style.color='#999'}" style="color:#999999" />请输入描述信息</textarea>
     	<!--
...
($_POST['description']=="")? $photo_desc = "这家伙很懒，什么也没有留下":$photo_desc = $_POST['description'];
($_POST['tags']=="请输入关键字")? $photo_tags = NULL:$photo_tags = $_POST['tags'];
  ...
  if ($_FILES["file"]["error"] > 0)
    ...
    if (file_exists($target . $_FILES["file"]["name"]))
      ...
      $insert = "INSERT INTO " . $table2photo .
            ...
            "VALUES ('" . get_option('album_folder_dir').$_FILES["file"]["name"] ."','".$select_album_ID. "','".get_option('album_folder_dir')."thumbs/".$_FILES["file"]["name"] ."','". $photo_desc ."','". $current_user->ID."','" . $datetime ."','".$photo_tags. "')";
